require([ 'layer','util'],function(layer,util){
	$(function(){
        $("#internList").bootstrapTable({ // 对应table标签 的id
            method: 'post',
            url: CONSTANT.root_url + "/admin/searchInternList", // 获取表格数据的url//条件start,length
            ...
            columns: [
                {
                    ...
                    formatter: function (value, row, index) {//field对应的数据,field对应的数据,0,1,2
                        var options = $("#internList").bootstrapTable('getOptions');
...
function  delEnt(id) {
    ...
    $.ajax({
        ...
        success : function(result) {
            layer.alert("删除成功");
            //刷新
            $("#internList").bootstrapTable("refreshOptions",{pageNumber:1,pageSize:10});//便可以了
        },
        error : function(){
            layer.msg("服务器出错!");
        }
